...
// Ambil data rekam medis forward chaining
$query = $conn->query("SELECT rm.*, a.nama_lengkap AS nama_pasien, a.jenis_kelamin, a.tanggal_lahir FROM rekam_medis rm JOIN tabel_akun a ON rm.id_akun = a.id_akun");
$rekam_medis_fc = $query->fetchAll(PDO::FETCH_ASSOC);

// Ambil data rekam medis certainty factor
$query = $conn->query("SELECT drm.*, a.nama_lengkap AS nama_pasien, a.jenis_kelamin, a.tanggal_lahir FROM detail_rekam_medis drm JOIN tabel_akun a ON drm.id_akun = a.id_akun");
...
function getNamaPenyakit($kode_penyakit, $conn) {
    $kode_array = array_map('trim', explode(',', $kode_penyakit)); // Hilangkan spasi ekstra
    $nama_penyakit_list = [];
...
<style>
   @media print {
    ...
    th, td {
        ...
        padding: 6px;
        text-align: center;
        word-wrap: break-word; /* Agar teks tidak menyebabkan perubahan ukuran */
    }
    ...
    .message-box {
        width: 100%; /* Pastikan pesan tidak mempengaruhi tabel */
        text-align: center;
        padding: 10px;
        margin-bottom: 10px;
    }
...
<?php if (isset($_SESSION['pesan'])): ?>
    <div class="message-box bg-blue-100 text-blue-700 rounded">
        <?= $_SESSION['pesan']; ?>
    </div>
    <script>
        setTimeout(function() {
...
<?php endif; ?>
    ...
    <main class="ml-64 p-6 w-full">
        <h1 class="text-3xl font-bold text-orange-800 mb-6">Rekam Medis</h1>
        
        

        <!-- Rekam Medis Certainty Factor -->
        <div class="bg-white p-6 rounded-lg shadow-lg">
            <h2 class="text-xl font-semibold text-orange-700 mb-4">Rekam Medis Certainty Factor</h2>
            <button onclick="printTable('table_cf')" class="mb-4 px-4 py-2 bg-orange-600 text-white rounded">Cetak</button>
            <table id="table_cf" class="w-full border-collapse border border-orange-300">
                <thead>
                ...
                <tbody>
                    <?php $no = 1; foreach ($rekam_medis_cf as $rm): ?>
                        <tr class="hover:bg-orange-100">
                            ...
                            <td class="border border-orange-300 p-3"><?= $rm['nama_pasien']; ?></td>
                            <td class="border border-orange-300 p-3 text-center"><?= $rm['jenis_kelamin']; ?></td>
                            <td class="border border-orange-300 p-3 text-center"><?= $rm['tanggal_lahir']; ?></td>
                            <td class="border border-orange-300 p-3"><?= getNamaPenyakit($rm['kode_penyakit'], $conn); ?></td>
                            <td class="border border-orange-300 p-3 text-center"><?= str_replace(',', '<br>', $rm['nilai_cf']); ?></td>
                            <td class="border border-orange-300 p-3"><?= $rm['solusi']; ?></td>
                            <td class="border border-orange-300 p-3 text-center"><?= $rm['tanggal_konsultasi']; ?></td>
                            <td class="border border-orange-300 p-3 text-center">
                                    <a href="aktivitas/hapus_rekammedis.php?id=<?= $rm['id_detail']; ?>" 
